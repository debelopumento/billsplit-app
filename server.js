...
app.get('/bills', (req, res) => {
  Bills.find()
    ...
    .then(bills => {
      res.json({
        bills: bills.map(bills => bills.apiRepr()),
      });
    })
    .catch(err => {
      console.error(err);
...
app.get('/bills-sum-2users/:userIdInput/:friendIdInput', (req, res) => {
  Bills.find({
    $or: [
      {
        $and: [
          { users: { $elemMatch: { userId: req.params.friendIdInput } } },
          { 'paidByUser.userId': req.params.userIdInput },
        ],
      },
      {
        $and: [
          { users: { $elemMatch: { userId: req.params.userIdInput } } },
          { 'paidByUser.userId': req.params.friendIdInput },
        ],
      },
    ],
  })
...
app.post('/bills', (req, res) => {
  ...
  Bills.create({
    ...
    const promises = updatedBill.users.map(function(localBillSplitter) {
      ...
      if (localBillSplitter.userId != updatedBill.paidByUser.userId) {
        ...
        return Bills.find({
          $and: [
            {
              users: { $elemMatch: { userId: updatedBill.paidByUser.userId } },
            },
            { 'paidByUser.userId': localBillSplitter.userId },
          ],
        })
          .exec()
          .then(function(billsCollectionA) {
            ...
            billsCollectionA.forEach(function(localBill) {
              localBill.users.forEach(function(localUser) {
                if (localUser.userId === updatedBill.paidByUser.userId) {
                  ...
                  billSplitterBalance = billSplitterBalance +
                    localUser.splitAmount;
                }
              });
            });
            ...
            return Bills.find({
              $and: [
                { users: { $elemMatch: { userId: localBillSplitter.userId } } },
                { 'paidByUser.userId': updatedBill.paidByUser.userId },
              ],
            })
              .exec()
              .then(function(billsCollectionB) {
                ...
                billsCollectionB.forEach(function(localBill) {
                  localBill.users.forEach(function(localUser) {
                    if (localUser.userId === localBillSplitter.userId) {
                      ...
                      billPayerBalance = billPayerBalance +
                        localUser.splitAmount;
                      billSplitterBalance = billSplitterBalance -
                        localUser.splitAmount;
                    }
                ...
                return User.findById(updatedBill.paidByUser.userId)
                  .exec()
                  .then(function(billPayer) {
                    ...
                    return User.findById(localBillSplitter.userId)
                      .exec()
                      .then(function(toBeUpdatedBillSplitter) {
                        toBeUpdatedBillSplitter.friends.forEach(
                          function(friend) {
                            if (
                              friend.userId === updatedBill.paidByUser.userId
                            ) {
                        ...
                        return User.findByIdAndUpdate(
                          localBillSplitter.userId,
                          toBeUpdatedBillSplitter
                        );
                      });
                  });
              });
    ...
    Promise.all(promises).then(bills => res.json(updatedBill)).catch(err => {
      console.error(err);
      res.status(500).json({ message: 'Internal server error' });
    });
  });
...
app.put('/bills/:id', (req, res) => {
  ...
  Bills//find old bill in database, name is oldBill.
  .findById(req.params.id)
    ...
    .then(function(oldBill) {
      ...
      Bills.findByIdAndUpdate(req.params.id, newBill)
        .exec()
            ...
            if (localBillSplitter.userId != updatedBill.paidByUser.userId) {
              Bills.find({
                $and: [
                  {
                    users: {
                      $elemMatch: { userId: updatedBill.paidByUser.userId },
                    },
                  },
                  { 'paidByUser.userId': localBillSplitter.userId },
                ],
                ...
                .then(function(billsCollectionA) {
                  ...
                  Bills.find({
                    $and: [
                      {
                        users: {
                          $elemMatch: { userId: localBillSplitter.userId },
                        },
                      },
                      { 'paidByUser.userId': updatedBill.paidByUser.userId },
                    ],
                    ...
                    .then(function(billsCollectionB) {
                      ...
                      return User.findById(updatedBill.paidByUser.userId)
                        ...
                        .then(function(billPayer) {
                          ...
                          let friendSubDoc = billPayer.friends.id(
                            friendSubDocId
                          );
                          friendSubDoc.balance = billSplitterBalance;
                          billPayer.save();
...
app.delete('/bills/:id', (req, res) => {
  Bills.findById(req.params.id)
    ...
    .then(function(updatedBill) {
      ...
      //remove bill
      Bills.findByIdAndRemove(updatedBill.id)
        .exec()
        .then(function(updatedBill) {
          //get IDs of users that need to be updated in database
          const promises = updatedBill.users.map(function(localBillSplitter) {
