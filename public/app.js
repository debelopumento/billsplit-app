function register(signedInUserFacebookId, signedInUserFullName) {
    ...
    var registrationForm = 'Username: <input class="js-userName" type="text" placeholder="username"><br>' +
        'Full Name:<input class="js-fullName" type="text" value="' +
...
function login(signedInUser) {
    ...
    function editBill(localBill, isNewBill) {
        ...
        function renderBillSplitterList() {
            ...
            localBill.users.forEach(function(billSplitter) {
                billSplitterListHtml += '<div splitIndex="' +
                    billSplitIndex +
                    '"><input id="splitter-index-' +
                    billSplitIndex +
                    '" class="autocompleteName" type="text" value="' +
                    billSplitter.fullName +
                    '"><input id="splitAmount-index-' +
                    billSplitIndex +
                    '" type="number" value="' +
                    billSplitter.splitAmount +
                    '"><button class="js-removeBillSplitter">Remove</button></div>';
                billSplitIndex++;
            });
        ...
        function updateUserList() {
            ...
            for (localIndex = 0; localIndex < billSplitterCount; localIndex++) {
                ...
                signedInUserFriendList.forEach(function(friendSearch) {
                    if (
                        friendSearch.fullName ===
                        localBill.users[localIndex].fullName
                    ) {
                        localBill.users[
                            localIndex
                        ].userId = friendSearch.userId;
                    }
                });
                var splitAmountId = '#splitAmount-index-' + localIndex;
                localBill.users[localIndex].splitAmount = $(
                    splitAmountId
                ).val();
            }
        }
        ...
        $('.js-editBillForm').append(
            '<div>Bill Date: <input class="billdate" type="date" value="' +
                formatDate(localBill.billDate) +
                '"></div>'
        );
        $('.js-editBillForm').append(
            '<div>Description: <input class="js-billDescription" type="text" value="' +
                localBill.description +
                '"></div>' +
                '<div>Total Amount: <input class="billTotalAmount" type="number" value="' +
                localBill.totalAmount +
                '"></div>' +
        ...
        $('.js-editBillForm').append(
            '<div>Paid by: <input id="paidByUser" class="autocompleteName" type="text" value="' +
                localBill.paidByUser.fullName +
                '"></div>'
        );
        //$('.js-editBillForm').append('<div>Due: <input class="billDueDay" type="date" value="' + formatDate(localBill.dueDay) + '"></div>');
        $('.js-editBillForm').append(
            '<div>Memo: <input class="billMemo memo" type="text" value="' +
                localBill.memo +
                '"></div>'
        );
        $('.js-editBillForm').append(
            '<div><button class="js-submitBillUpdates bigButton">Submit</button></div>'
        );
        ...
        $('.js-submitBillUpdates').click(function(event) {
            ...
            if (splitTotal != localBill.totalAmount) {
                alert(
                    'The total of splits does not equal to the total amount of the bill!'
                );
                billIsValid = false;
    ...
    function formatDate(unformattedDate) {
        ...
        if (formattedDay < 10) {
            ...
            formattedDay = formattedDay.toString();
        }
        ...
        function displayBill(bill) {
            ...
            $('main').append(
                '<p>Date: ' +
                    ...
                    bill.description +
                    '</p>' +
                    '<p>Total Amount: <span class="blue">$' +
                    bill.totalAmount +
                    '</span></p>'
            ) + '<p>Bill splits: </p>';
    ...
    function displayBillsWfriend(friendId, friendName) {
        ...
        function displayTransactionHistories(data, friendId) {
                ...
                if (localBill.paidByUser.userId != userId) {
                    localBill.users.forEach(function(user) {
                        if (user.userId === userId) {
                            ...
                            $('main').append(
                                '<div class="transaction" billId="' +
                                    localBill.id +
                                    '"><container class="transactionDetails"><span class="date left">' +
                                    formatDate(localBill.billDate) +
                                    '   </span><span class="billdescription right">' +
                                    localBill.description +
                                    '   </span></container></br><container class="transactionLine bottom"><span class="billSplit">You owe: <span class="red">$' +
                                    userOwe +
                                    '</span></span><button class="js-checkBillDetails right">Edit/See Details</button><br></container></div>'
                            );
                        ...
                        if (user.userId === friendId) {
                            ...
                            $('main').append(
                                '<div class="transaction" billId="' +
                                    ...
                                    '   </span><span class="billdescription right">' +
                                    localBill.description +
                                    '   </span></container></br><container class="transactionLine bottom"><span class="billSplit">' +
                                    friendName +
                                    ' owes you: <span class="green">$' +
                                    friendOwe +
                                    '</span></span><button class="js-checkBillDetails right">Edit/See Details</button><br></container></div>'
                            );
            ...
            if (balance === 0) {
                $('main').append(
                    '<div class="transactionTotle"><p>You two are even.</p></div>'
                );
            }
            if (balance > 0) {
                $('main').append(
                    '<div class="transactionTotle"><p>' +
                        friendName +
                        ' owes you: <span class="green">$' +
                        balance +
                        '</span></p></div>'
            ...
            if (balance < 0) {
                $('main').append(
                    '<div class="transactionTotle"><p>You owe ' +
                        friendName +
                        ' <span class="red">$' +
                        -balance +
                        '</span></p></div>'
    ...
    function addNewFriend() {
        ...
        $('.js-submit').click(function(event) {
            ...
            var url_username = window.location.href +
                'users/username/' +
                searchUsername;
    ...
    function displayBillSplitsSummary() {
        $.ajax({
            ...
            success: function(data) {
                    ...
                    if (friend.balance < 0) {
                        $('main').append(
                            '<div userFriendId="' +
                                ...
                                ' owes you:  <span class="green">$' +
                                -friend.balance +
                                '   </span><button class="js-checkFriendBillLog right">See Log</button></div>'
                        );
                        ...
                        $('main').append(
                            '<div userFriendId="' +
                                ...
                                '">' +
                                friend.fullName +
                                ' lends deposit you:  <span class="red">$' +
                                friend.balance +
                                '   </span><button class="js-checkFriendBillLog right">See Log</button></div>'
                        );
                    }
                ...
                $('main').append(
                    '<div class="buttonContainer"><button class="js-addNewFriend bigButton">Add a new user to your friend list</button></br><button class="js-addNewBill bigButton">Add a new bill</button></div>'
                );
